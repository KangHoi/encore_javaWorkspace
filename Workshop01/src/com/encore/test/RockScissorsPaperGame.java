...
public class RockScissorsPaperGame {
	...
	public static int game() { 
		// 가위바위보 게임 메소드 
		System.out.println("가위(1)바위(2)보(3) 하나 중 입력: ");
		Scanner sc = new Scanner(System.in);
		int user = sc.nextInt(); // 사용자가 "가위", "바위", "보" 중 입력
		int com = (int) (Math.random() * 3 +1); // 컴퓨터가 1~3 랜덤 선택
		
		if (user == com) { // 비길 경우 0을 리턴
			System.out.println("비겼습니다!!!");
			return 0;
		} else if ((user==1&& com==3) || (user==2&&com==1) || (user==3&&com==2)) { // 사용자가 이길 경우 1을 리턴
