 ...
 * 정렬이란?
 * 정렬전{3,6,1,2,8,9} ---> 오름차순 정렬 후 {1,2,3,6,8,9}  ---> 내림차순 정렬 후{9,8,6,3,2,1}
 * 
 * 정렬 | 순서 
 * 순서는 값이 입력된 순서를 말한다.
 * a, bb, a, c, zz
 ...
 * 5,3 두 수를 비교 --> 1)temp라는 임시 변수 하나 만듦
 * 						그 안에 5를 잠시 저장
 * 					  2) 3을 5자리에 가져다놓는다.
 * 					  3) 3 자리에는 temp 안에 있는 5를 다시 가져와서 갖다놓는다
 * 						---> 이렇게 두 수의 자리를 temp 변수를 이용해 바꾸는 방법을 swap이라 한다.
 */
